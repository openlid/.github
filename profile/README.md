...
**Tools that keep your Mac out of your way.**

</div>

We build small, focused macOS utilities. Native AppKit. No telemetry.
No data leaves your machine. Apache 2.0 licensed.
...
A menu-bar app that prevents your Mac from sleeping when you close the
lid (and from dimming when the lid is open). Native AppKit UI, a
first-class CLI, signed and notarized DMG distribution.
...
- **Native, not Electron.** Rust + AppKit via `objc2`. Single-digit
  megabyte binaries. Cold-launch in under a second.
- **No telemetry. No analytics. No automatic update checks.** Your
  ...
  we like it that way.
- **Source available, source clean.** All code published under
  Apache 2.0. Read it, fork it, improve it, redistribute it.
